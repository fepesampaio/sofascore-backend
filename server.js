...
// Configurações do endpoint do Smartproxy (extraídos do seu print)
 ...
 * Transforma uma chamada GET interna em uma chamada POST para a Decodo.
 */
function get(endpoint, params = {}) {
  return new Promise((resolve, reject) => {
    // 1. Montamos a URL alvo do SofaScore que queremos extrair
    const query = new URLSearchParams(params).toString();
    const targetUrl = `https://api.sofascore.com/api/v1${endpoint}${query ? "?" + query : ""}`;

    // 2. Preparamos o corpo da requisição conforme as configurações do seu teste
    const postData = JSON.stringify({
      url: targetUrl,
      proxy_pool: "premium", // Usando o pool premium para evitar o erro 403
      headless: "html"       // Garante a extração limpa do conteúdo
    });

    const options = {
      hostname: SCRAPER_API_URL,
      path: SCRAPER_API_PATH,
      method: "POST", // A API de Scraping exige método POST
      headers: {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end", () => {
        ...
        try {
          ...
          /**
           * A API do Smartproxy retorna o resultado do SofaScore dentro de 'content'.
           * Como o SofaScore retorna JSON, a Smartproxy nos entrega uma String JSON.
           */
          const targetContent = response.content || response;
          
          // Se o conteúdo for uma string, fazemos o segundo parse para objeto
    ...
    // Enviamos os dados da requisição
    req.write(postData);
    req.end();
  });
}
