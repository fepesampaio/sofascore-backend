...
export function ExtractionPanel() {
  ...
  const handleCopyLogs = () => {
    ...
    navigator.clipboard.writeText(textToCopy).then(() => {
      setCopied(true);
      toast({ title: "Copiado!", description: "Logs copiados para a área de transferência." });
            ...
            <div className="flex items-center justify-between border-b px-4 py-2 bg-card">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setIsTerminalExpanded(!isTerminalExpanded)}>
                  {isTerminalExpanded ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
                </button>
              </div>
            </div>
            {isTerminalExpanded && (
              <div ref={terminalRef} className="flex-1 overflow-y-auto bg-terminal-bg p-3 pr-4 terminal-scroll scroll-smooth">
                {logs.length === 0 ? (
                  <p className="text-xs text-gray-600 italic">Aguardando dados...</p>
                ) : (
                  logs.map((log, i) => (
                    <div key={i} className="flex gap-2 font-mono text-xs leading-relaxed mb-0.5">
                      {log.time && <span className="text-gray-600 shrink-0 select-none">[{log.time}]</span>}
                      <span className={cn(levelColor(log.level), "break-all")}>{log.message}</span>
                    </div>
                  ))
                )}
              </div>
