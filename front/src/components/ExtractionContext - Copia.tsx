...
interface LogEntry { time: string; level: "info" | "warn" | "error" | "success" | "system"; message: string; }
type PipelineStep = { label: string; subtitle: string; status: "pending" | "running" | "done" | "error"; };
...
export function ExtractionProvider({ children }: { children: React.ReactNode }) {
  ...
  const [logs, setLogs] = useState<LogEntry[]>([{ time: "", level: "system", message: "[SYSTEM]: Central de Comando pronta." }]);
